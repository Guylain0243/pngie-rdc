-- ============================================================================
-- PNGIE-RDC — Référentiel National des Tribunaux du Travail (RNTT)
-- Tome J9. Table vide à l'initialisation, aucune hypothèse sur le nombre
-- total, alimentation progressive après vérification des actes officiels.
--
-- Modèle institutionnel de référence (Tome J9, sections 3-5) :
--   PRESIDENCE (Président)
--   SIEGE      (Juges)
--   ASSESSEURS (représentant employeurs et travailleurs, participent au jugement)
--   GREFFE     (Greffier en Chef, Greffiers)
-- Ce modèle n'est pas instancié ici — seul le référentiel est créé.
--
-- Exécution : psql -f .\creer_referentiel_tribunaux_travail.sql $env:PNGIE_ADMIN_DB_URL
-- ============================================================================

BEGIN;

CREATE TABLE IF NOT EXISTS ref_tribunal_travail (
    ini                       VARCHAR(20) PRIMARY KEY,  -- ex: TTRAV-001
    ...
    liste_assesseurs          TEXT,          -- champ libre : assesseurs employeurs/travailleurs désignés
    president_nom             TEXT,
...
CREATE TABLE IF NOT EXISTS ref_tribunal_travail_historique (
...
SELECT COUNT(*) AS nb_ttrav_enregistres FROM ref_tribunal_travail;

COMMIT;
\echo '=== RNTT créé (Tome J9) — table vide, aucun Tribunal du Travail enregistré, aucune hypothèse de nombre ==='
