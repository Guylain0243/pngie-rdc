...
async function traiterCompte(c, passwordHash) {
  ...
  if (!personne) {
    ...
    console.log("  DEBUG valeurs personne:", JSON.stringify({ nom: c.nom, prenom: c.prenom, email: c.email }));
    await db.run(
...
async function main() {
  ...
  for (const c of COMPTES) {
    try {
      ...
      console.log("  Colonne:", e.column || "aucune");
      console.log("  Contrainte:", e.constraint || "aucune");
      console.log("  Table:", e.table || "aucune");
    }
  }
  console.log("`n===== TERMINE (tous les comptes ont ete tentes) =====");
  process.exit(0);
}
