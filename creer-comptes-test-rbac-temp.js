...
async function main() {
  ...
  if (!password) {
    console.log("ERREUR : definissez $env:PNGIE_TEST_PASSWORD avant de lancer ce script");
    process.exit(1);
  }
  ...
  for (const c of COMPTES) {
    ...
    if (!institution) { console.log("  ERREUR : institution " + c.institutionCode + " introuvable"); continue; }

    let unite = await db.get(
      "SELECT unite_id FROM unite_organisationnelle WHERE institution_id = ? AND code = ?",
      [institution.institution_id, "TEST_RBAC"]
    );
    if (!unite) {
      const uniteId = crypto.randomUUID();
      await db.run(
        `INSERT INTO unite_organisationnelle (unite_id, institution_id, code, nom, type_unite, niveau_hierarchique, statut)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
    ...
    let poste = await db.get(
      "SELECT poste_id FROM poste WHERE unite_id = ? AND code = ?",
      [unite.unite_id, "TEST_RBAC"]
    );
    if (!poste) {
      const posteId = crypto.randomUUID();
      await db.run(
        `INSERT INTO poste (poste_id, unite_id, code, intitule, niveau_hierarchique, categorie, statut, nombre_postes_autorises)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    ...
    if (!personne) {
      ...
      await db.run(
        `INSERT INTO person (person_id, nom, email, password_hash, statut) VALUES (?, ?, ?, ?, ?)`,
        [personneId, c.prenom + " " + c.nom, c.email, passwordHash, "ACTIF"]
      );
      console.log("  Personne creee (person + personne, meme UUID)");
    } else {
      personneId = personne.personne_id;
      await db.run("UPDATE personne SET password_hash = ? WHERE personne_id = ?", [passwordHash, personneId]);
      await db.run("UPDATE person SET password_hash = ? WHERE person_id = ?", [passwordHash, personneId]);
