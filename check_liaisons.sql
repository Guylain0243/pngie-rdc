SELECT pr.scope_institution_id, pr.personne_id, pr.statut, p.poste_id, p.intitule, p.niveau_hierarchique
FROM poste p
LEFT JOIN personne_role pr ON pr.scope_institution_id = p.unite_id
WHERE p.poste_id IN (
...
-- Verifier aussi si des postes ont un lien direct via une colonne titulaire sur poste lui-meme
SELECT column_name FROM information_schema.columns WHERE table_name = 'poste' AND column_name ILIKE '%personne%' OR column_name ILIKE '%titulaire%';
