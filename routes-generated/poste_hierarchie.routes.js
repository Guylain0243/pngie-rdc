...
router.get("/postes/arborescence", exigerPermission("unite_organisationnelle", "READ"), async (req, res) => {
  try {
    ...
    const placeholders = institutionsVisibles.map(() => "?").join(",");
    const rows = await db.all(`
      ...
      WHERE u.statut = ? AND p.statut = ? AND i.institution_id IN (${placeholders})
      ORDER BY i.nom, u.niveau_hierarchique, p.niveau_hierarchique, p.intitule
    `, ["ACTIF", "ACTIF", ...institutionsVisibles]);
